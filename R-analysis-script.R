...
my.devices <-  factor(unlist(sapply(my.files, read.csv, header=FALSE, sep="\t", simplify = TRUE, skip = "2", colClasses=c("NULL", NA), nrows = 1), recursive = TRUE), ordered = TRUE)
...
m1<-as.table(cbind(my.devices,PCEvec))
m2 <-split.data.frame(m1, my.devices, drop = TRUE)
m3 <-as.table(m2$OTH0001)
